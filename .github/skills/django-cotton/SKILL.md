...
### Manual Discovery (if script not available)

If the script is not accessible, you can manually discover components:
...
for loader in engine.engine.template_loaders:
    if hasattr(loader, 'get_dirs'):
        for template_dir in loader.get_dirs():
            ...
            if cotton_path.exists():
                for f in cotton_path.rglob('*.html'):
                    rel = f.relative_to(cotton_path).with_suffix('')
                    name = str(rel).replace('\\', '.').replace('/', '.').replace('_', '-')
